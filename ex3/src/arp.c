...
arp_request(pcap_t *fp, uint8_t *ip)
{
	...
	COPY_ETH_ADDR(pkt.eth_dst, ethbroadcast);
	...
	pkt.arp_iplen = 4;
	pkt.arp_op = 0x0100;
	COPY_ETH_ADDR(pkt.arp_srceth, myethaddr);
	setip(pkt.arp_srcip, myipaddr);
	COPY_ETH_ADDR(pkt.arp_dsteth, ethnull);	
...
arp_reply(pcap_t *fp, uint8_t *dsteth, uint8_t *dstip)
{
	...
	COPY_ETH_ADDR(pkt.arp_srceth, myethaddr);
	setip(pkt.arp_srcip, myipaddr);
	COPY_ETH_ADDR(pkt.arp_dsteth, dsteth);
